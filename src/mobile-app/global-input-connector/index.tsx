...
const GlobalInputConnector = ({ codedata, toCameraView, menuItems }) => {
  ...
  const [action, setAction] = useState<any>(formData.loadingData);

  useEffect(() => {
    const onDisconnected = () => setAction(formData.finishSessionAction);
    const onInput = (message) => setAction((action) => formData.onInputAction(message, action));
    const onDeviceConnected = (initData) => setAction((action) => formData.initDataAction(initData));
  ...
  const getGlobalInputPairingData = () => globalInput.getPairingData({ globalInputConnector });
  ...
  const fillContentEncryptionForm = (content, label) =>
    formData.fillContentEncryptionForm({ content, label, setAction, onFieldChanged: sendFieldToDevice });
  ...
  const renderExportEncryptionKey = () => (
    <ExportEncryptedEncryptionKey onCancel={toDeviceInput} onCompleted={fillContentEncryptionForm} />
  );
  ...
  const onPairingData = () => {
    try {
      fillContentEncryptionForm(getGlobalInputPairingData(), settingsTextConfig.serviceData.title);
    } catch (error) {
      formData.printError({
  ...
  const renderContent = () => {
    switch (action.actionType) {
      ...
      case ACT_TYPE.ENCRYPT_SECRET:
        return formData.renderEncryptSecret({ action, setAction, onFieldChanged: sendFieldToDevice, messageTimerHandler });
      ...
      case ACT_TYPE.ENCRYPT_SELECT_KEY:
        return encryptDecryptData.renderEncryptSelectKey({ action, setAction, onDisconnect });
      case ACT_TYPE.DECRYPT_SELECT_KEY:
        return encryptDecryptData.renderDecryptSelectKey({ action, setAction, onDisconnect });
