...
<FirstSection>
              ...
              <P>
              The data stored in the mobile app can be any data. For example, it can be a set of parameters representing the user’s viewing habit in a media application. The parameters are re-calculated after each session and present the user with an option to save them back into the mobile app. The media application can request the data on-demand as part of the login process so that the user can push it from the mobile app to the application to enjoy the personalized user experience. This may make some users happy if they prefer to keep their viewing habits from others sharing the same account.
              </P>
              <P>
              The same approach is used to automate the subscription, sign in and any other operations in the applications running on the big screen devices. The application can request the necessary data using end-to-end encrypted communication. The user can then inspect the selected data and then push it to the application. Please visit the Global Input App website for more information on this.
              </P>
              <P>
...
A form will be displayed on your mobile titled with <Concept>Our Contact Details</Concept>, you can now press the <Concept>Save</Concept> button to save our details into your app. You can view/edit the data item you have saved on the <Concept>Data</Concept> tab.
If you can now scroll to the bottom of the form, and press the <Concept>Continue</Concept> button there, a new form for sending a message to us will be displayed. If you fill in your details and the content of the message, and then press the “Send Now”, a message will be sent to us. Now you need to save the form data you have just entered into your app. If you reload the page and go through the steps again, you will notice that a button with the <Concept>Matched</Concept> label is displayed on the bottom of your mobile screen. This means that at least one data item matching the form exists in your app. Press on it to see the list and select the row to display its details. The data details will be displayed with <Concept>*</Concept> characters. This means that the data is not decrypted yet. You can press the <Concept>Show</Concept> button to decrypt and show its details or press the “Select” button to send the data item straight away to the form being displayed on your computer. Now you can just need to place a new message and press the <Concept>Send Now</Concept> button.
              </P>

              <P>
              <Title2>A Privacy Compliance Solution</Title2>
              Many applications need to store user data into the application databases in order to provide a personalised experience. This means the user data will be collected in the application databases. In this case, in order to comply with the privacy policy, application developers may have to spend quite significant amount of effort on protecting the user data and making sure that the user has given consent on collecting the personal data and how to use the user data etc.
...
For example, a media application may need to personalise the user experience using the preference/user habits parameters that can be automatically adjusted in each session by following what programmes that the user has watched. This data be can be stored as part of the authentication data in the Global Input App with the choice of the user, and the user may choose to send the data back to the application to get the personalized experience, thus eliminating the need for the application to store the user-specific data on the server. This may simplify the development of the application and may be appealing to the users who may prefer the application not to store his/her viewing habits into databases.
              </P>
              <P>
              <Title2>Subscription on Smart TV</Title2>
              A Smart TV media application may require users to create an account before using it. Using the TV remote controller to sign in is already a time-consuming task, let alone going through the entire subscription process. The Smart TV media application can be easily extended to have mobile input and mobile control functionality, so that a user can use his/her mobile to carry out “subscription”, “sign in”, search and browsing functionalities.
              </P>
