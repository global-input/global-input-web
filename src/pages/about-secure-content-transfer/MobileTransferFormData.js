...
const {TransferFormDataListExample} = examplesLinks;
...
const MobileTransferFormData=props=>{
    ...
    return(
         <React.Fragment>
          ...
          <NextSection>
          <P>
            Transferring content between user mobile and applications securely and allowing the user to save and load the data on-demand is an attractive option when it comes to personal data or other sensitive information.
          </P>
          <P>
                      The communication between the mobile app and the application is secured with the end-to-end encryption, and it is initiated by scanning an encrypted QR code. The QR Code contains a one-time-use encryption key for initiating the end-to-end encrypted communication. The Global Input App scans the QR code to obtain the encryption key, which is generated by the application on-the-fly. This means that the key is captured via camera instead of transferring electronically. Hence, even if the HTTPS communication is compromised by using a compromised CA (Certificate Authority) and an active MitM (man-in-the-middle) attack, the communication will be still secure.
          </P>
          <P>
            The communication between the mobile app and the application is secured with the end-to-end encryption, and it is initiated by scanning an encrypted QR code. The QR Code contains a one-time-use encryption key for initiating the end-to-end encrypted communication. The Global Input App scans the QR code to obtain the encryption key, which is generated by the application on-the-fly. This means that the key is captured via camera instead of transferring electronically. Hence, even if the HTTPS communication is compromised by using a compromised CA (Certificate Authority) and an active MitM (man-in-the-middle) attack, the communication will be still secure.
...
Also, the data will always stay encrypted until to the point of actual use and the decrypted data will be discarded immediately after the use, minimizing the reliance on the system security.
Confidential data such as user credentials, personal information, user preferences for personalized experiences etc. can be stored in the mobile storage securely. The connected application can send data to the user for storing for later use. When this happens, the user will be presented with an option of saving the data into the storage. Later when the application requires the data, he/she can select and inspect the selected data content and push it to the application on demand.
          ...
          <P>
                        The following code is an example of a form that can transfer data via the mobile app:
          </P>
